...
function repeatString(str, times) {
  return times > 0 ? str + str.repeat(times - 1) : '';
}
...
function removeFirstOccurrences(str, value) {
  const occurencesIndex = str.indexOf(value);

  if (occurencesIndex === -1) {
    return str;
  }

  return (
    str.substring(0, occurencesIndex) +
    str.substring(occurencesIndex + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const occurencesIndex = str.lastIndexOf(value);
...
function countVowels(str) {
  return str.length - str.replaceAll(/[aeiouy]/gim, '').length;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < symbols.length / 2; i += 1) {
    if (
      symbols[i].toUpperCase() !== symbols[symbols.length - 1 - i].toUpperCase()
    ) {
      return false;
    }
  }
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((value) => encrypted[alphabet.indexOf(value)] || value)
    .join('');
}
...
function getCardId(value) {
  ...
  return (
    cards.indexOf(value.substring(0, value.length - 1)) +
    13 * suits.indexOf(value[value.length - 1])
  );
}
